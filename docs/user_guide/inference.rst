...
The ``kraken-gpu`` command-line interface (CLI) is the primary entry point for all
...
.. code-block:: bash
   ...
   $ kraken-gpu [global-options] subcommand [subcommand-options] ...
...
.. code-block:: bash
   ...
   $ kraken-gpu -i input.jpg output.txt segment -bl ocr -m model.safetensors
...
``-i, --input <src> <dest>``
    ...
    .. code-block:: bash
       ...
       $ kraken-gpu -i page1.tif page1.xml -i page2.tif page2.xml segment -bl
...
``-o, --suffix <suffix>``
    ...
    .. code-block:: bash
       ...
       $ kraken-gpu -I "data/*.png" -o .txt ocr -m model.mlmodel
...
.. code-block:: bash
   ...
   $ kraken-gpu -i page.tif page_bin.png binarize --threshold 0.5
...
.. code-block:: bash
   ...
   $ kraken-gpu -a -i page.tif page.seg.json -n segment -bl

.. code-block:: bash

   # Legacy bbox segmentation (requires binary input)
   $ kraken-gpu -i page_bin.png page_bbox.json -n segment -x
...
.. code-block:: bash
   ...
   $ kraken-gpu -i page.tif page.xml -a segment -bl ocr -m model.safetensors

.. code-block:: bash

   # Recognize pre-segmented XML files
   $ kraken-gpu -f xml -I "data/*.xml" -o _ocr.xml ocr -m specialized.safetensors

.. code-block:: bash

   # Batched + parallelized recognition (GPU)
   $ kraken-gpu -i page.tif page.txt --device cuda:0 --precision bf16-mixed \
