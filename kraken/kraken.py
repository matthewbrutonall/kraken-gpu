...
@click.group(context_settings=dict(show_default=True,
             ...
             chain=True)
@click.version_option(package_name='kraken-gpu')
...
@click.option('-w', '--workers', 'workers', default=1, type=click.IntRange(1), help='Number of multiprocessing workers for batch segmentation')
...
def cli(**kwargs):
    ...
    ctx.meta['workers'] = params['workers']
...
def process_pipeline(subcommands, input, batch_input, suffix, verbose, format_type, pdf_format, **args):
    ...
    # TIER 1 HARD FORK BATCH SEGMENTATION
    workers = ctx.meta.get('workers', 1)
    if len(subcommands) == 1 and getattr(subcommands[0], 'func', None) and subcommands[0].func.__name__ == 'segmenter':
        legacy, model, config = subcommands[0].args
        if not legacy and len(input) > 1 and workers > 1:
            from kraken.batch_segmenter import run_batch
            run_batch(input, model, config, ctx)
            return
