...
def _gaussian_kernel1d(sigma, order, radius):
    """Match scipy.ndimage._filters._gaussian_kernel1d."""
...
def _correlate_1d(img, kernel, axis):
    """Correlate a NCHW tensor with a 1-D kernel along H (axis=0) or W (axis=1).

    Uses flipped conv weights so the result matches scipy.ndimage correlation.
    """
    k = torch.as_tensor(kernel, dtype=img.dtype, device=img.device)
    pad = k.numel() // 2
    if axis == 0:
        w = k.flip(0).view(1, 1, -1, 1)
        return F.conv2d(img, w, padding=(pad, 0))
    w = k.flip(0).view(1, 1, 1, -1)
    return F.conv2d(img, w, padding=(0, pad))


def sato_torch(image, sigmas=(1, 3, 5, 7, 9), black_ridges=False, device=None):
    """Sato tubeness filter matching skimage.filters.sato(..., mode='constant').

    Intended for the parent process only. Do not call from forked CPU workers.
    """
    if device is None:
    ...
    if not black_ridges:
        img = -img
    t = torch.from_numpy(np.ascontiguousarray(img)).to(device).unsqueeze(0).unsqueeze(0)
    filtered_max = torch.zeros_like(t)
    sq1_2 = 1.0 / np.sqrt(2.0)
    for sigma in sigmas:
        sigma_scaled = sq1_2 * float(sigma)
        truncate = 8.0 if sigma_scaled > 1.0 else 100.0
        radius = int(truncate * sigma_scaled + 0.5)
        k0 = _gaussian_kernel1d(sigma_scaled, 0, radius)
        k1 = _gaussian_kernel1d(sigma_scaled, 1, radius)
        # First-order gradients: order [1,0] and [0,1]
        grad_r = _correlate_1d(_correlate_1d(t, k1, axis=0), k0, axis=1)
        grad_c = _correlate_1d(_correlate_1d(t, k0, axis=0), k1, axis=1)
        # Hessian via a second first-order pass (skimage._hessian_matrix_with_gaussian)
        Hrr = _correlate_1d(_correlate_1d(grad_r, k1, axis=0), k0, axis=1)
        Hrc = _correlate_1d(_correlate_1d(grad_r, k0, axis=0), k1, axis=1)
        Hcc = _correlate_1d(_correlate_1d(grad_c, k0, axis=0), k1, axis=1)
        tmp1 = (Hrr + Hcc) * 0.5
        tmp2 = torch.sqrt(((Hrr - Hcc) * 0.5) ** 2 + Hrc ** 2)
        l_max = tmp1 + tmp2
        vals = (float(sigma) ** 2) * torch.clamp(l_max, min=0)
        filtered_max = torch.maximum(filtered_max, vals)
    return filtered_max.squeeze().detach().cpu().numpy()


def apply_sato_heatmap(heatmap, cls_map, device=None):
    """Apply Sato to each baseline channel. Parent process only."""
    out = np.array(heatmap, copy=True)
    for _name, idx in cls_map['baselines'].items():
        out[idx] = sato_torch(out[idx], device=device)
    return out
...
def vectorize_lines(im: np.ndarray, threshold: float = 0.17, min_length=5,
    ...
    Args:
        ...
        skip_ridge_filter (bool): If True, `im[2]` is already a Sato response
                             (applied in the parent process). Workers must pass
                             True so they never call CUDA.
