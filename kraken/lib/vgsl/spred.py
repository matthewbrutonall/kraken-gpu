...
def vec_lines(heatmap: torch.Tensor,
    ...
    for bl_type, idx in cls_map['baselines'].items():
        ...
        baselines.extend([(bl_type, x) for x in vectorize_lines(
            heatmap[(st_sep, end_sep, idx), :, :],
            text_direction=text_direction[:-3],
            skip_ridge_filter=kwargs.get('sato_applied', False))])
